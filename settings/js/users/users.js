...
var UserList = {
	...
	getNemAddress: function (element) {
		return ($(element).closest('tr').data('nemAddress') || '').toString();
	},
...
$(document).ready(function () {
	...
	// by Jun Salinga
	if ($('#CheckboxNemAddress').is(':checked')) {
		$("#userlist .nemAddress").show();
	}
	// Option to display/hide the "NEM Address" column
	$('#CheckboxNemAddress').click(function () {
		if ($('#CheckboxNemAddress').is(':checked')) {
			$("#userlist .nemAddress").show();
			if (OC.isUserAdmin()) {
				OCP.AppConfig.setValue('core', 'umgmt_show_nem', 'true');
			}
		} else {
			$("#userlist .nemAddress").hide();
			if (OC.isUserAdmin()) {
				OCP.AppConfig.setValue('core', 'umgmt_show_nem', 'false');
			}
		}
	});
	// end: by Jun Salinga
